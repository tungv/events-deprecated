...
describe('e2e', () => {
  ...
  it('should insert', async () => {
    ...
    try {
      ...
      await db.dropCollection('stories_v1.0.0');
    ...
    const requests = [
      {
        ...
        projections: {
          users: [
            ...
            { __pv: '1.1.0', op: { insert: [{ full_name: 'Test 1' }] } },
          ],
          stories: [{ __pv: '1.0.0', op: { insert: [{ story_name: 'abc' }] } }],
        },
      },
      {
    ...
    expect(await dispatch(requests)).toEqual({
      changes: 4,
      requests,
    });
  });
